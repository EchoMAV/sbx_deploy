...
# 
# This is a simple video service currently handling a single stream
# Reads CAMERA_TYPE from the config file, currently supports IMX477, BOSON640, BOSON320
...
if [[ $CAMERA_TYPE == "IMX477" ]]; then
    ...
    for device in $video_devices; do
        ...
        if [[ "$device_name" == *"Boson"* ]]; then
            ...
            gst-launch-1.0 v4l2src device=$device io-mode=mmap ! "video/x-raw,format=(string)I420,width=(int)640,height=(int)512,framerate=(fraction)30/1" ! videorate max-rate=30 skip-to-first=true ! videoscale method=bilinear name=scale ! "video/x-raw,format=(string)I420,width=(int)1080,height=(int)608,framerate=(fraction)30/1" ! v4l2h264enc extra-controls="controls,video_bitrate=${SCALED_LOS_BITRATE}" ! "video/x-h264,level=(string)4.2" ! rtph264pay config-interval=1 pt=96 ! udpsink host=${LOS_HOST} port=${LOS_PORT} sync=false
        fi
    done
elif [[ $CAMERA_TYPE == "BOSON320" ]]; then
    video_devices=$(ls /dev/video*)
    # Loop through each video device and check if it is a FLIR Boson camera
    for device in $video_devices; do
        # Use v4l2-ctl to get the device name
        ...
        if [[ "$device_name" == *"Boson"* ]]; then
            echo "FLIR Boson camera found at: $device"       
            gst-launch-1.0 v4l2src device=$device ! v4l2h264enc extra-controls="controls,video_bitrate=${SCALED_LOS_BITRATE}" ! "video/x-h264,level=(string)4.2" ! rtph264pay config-interval=1 pt=96 ! udpsink host=${LOS_HOST} port=${LOS_PORT} sync=false
         fi
    done
fi
